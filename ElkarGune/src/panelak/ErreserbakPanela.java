...
import eragiketak.aldaketaErreserbak;
import kudeaketak.kudeaketaErreserba;
import objetuak.Erreserba;
import taulak.ErreserbakTaula;



public class ErreserbakPanela extends JFrame {

	private static final long serialVersionUID = 1L;
	private JPanel contentPane;
	private JTable table;
	private kudeaketaErreserba dao;
	private JTextField txt_id;

	public static void main(String[] args) {
		EventQueue.invokeLater(() -> {
			try {
				ErreserbakPanela frame = new ErreserbakPanela();
	...
	public ErreserbakPanela() {
		...
		dao = new kudeaketaErreserba();
		List<Erreserba> lista = dao.lortuErreserba();
		...
		ezabatuIkonoa.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					int idErreserba = Integer.parseInt(txt_id.getText().trim());
					Optional<Erreserba> erreserba = lortuErreserbaIdarekin(idErreserba);
					if (erreserba.isPresent()) {
						Object[] options = { "Bai", "Ez" }; // Bai edo ez izateko aukerak ematen ditu
						int erantzuna = JOptionPane.showOptionDialog(null, "Erreserba ezabatu nahi duzu?",
								"EZABATU", JOptionPane.YES_NO_OPTION, +JOptionPane.QUESTION_MESSAGE, null,
								options, options[0]);
						if (erantzuna == JOptionPane.YES_OPTION) {
							ezabatuErreserba(idErreserba);
							actualizarTabla();
						}
					} else {
						JOptionPane.showMessageDialog(null, "Erreserba ez da aurkitu ID honekin: " + idErreserba,
		...
		lblAldatu.addMouseListener(new MouseAdapter () {
            ...
            public void mouseClicked(MouseEvent e) {
                // ID-a hartu
                try {
                    int idErreserba = Integer.parseInt(txt_id.getText());
                    // Mezua bilatu ID-a erabiliz
                    Optional<Erreserba> erreserbakOpt = lortuErreserbaIdarekin(idErreserba);
                    
                    if (erreserbakOpt.isPresent()) {
                        Erreserba erreserba = erreserbakOpt.get();
                        // Hornitzailea aurkitu bada, datuak pasatzea eta AldaketaHornitzaileak erakustea
                        dispose();
                        aldaketaErreserbak ae = new aldaketaErreserbak(erreserba);
                        ae.setVisible(true);  // Erakutsi
                    } else {
                        // Hornitzailea ez bada aurkitu, errore-mezua erakutsi
                        JOptionPane.showMessageDialog(null, "Erreserba ez da aurkitu ID honekin: " + idErreserba, "Errorea", JOptionPane.ERROR_MESSAGE);
                    }
                } catch (NumberFormatException ex) {
				...
				lblAtzera.addMouseListener(new MouseAdapter () {
					...
					public void mouseClicked(MouseEvent e) {
						...
						lblFondoa.setIcon(new ImageIcon(ErreserbakPanela.class.getResource("/media/FONDOAK_sinS.png")));
	...
	private Optional<Erreserba> lortuErreserbaIdarekin(int idErreserba) {
		return dao.lortuErreserba().stream().filter(erreserba -> erreserba.getIdErreserba() == idErreserba).findFirst();
	}

	private void ezabatuErreserba(int idErreserba) {
		boolean isDeleted = dao.ezabatuErreserba(idErreserba);
		if (!isDeleted) {
			txt_id.setText("");
			JOptionPane.showMessageDialog(null, "Errore bat gertatu da. Erreserba ezin izan da ezabatu.", "Errorea",
					JOptionPane.ERROR_MESSAGE);
		} else {
			txt_id.setText("");
			JOptionPane.showMessageDialog(null, "Erreserba ezabatua izan da.");
		}
	}
}
